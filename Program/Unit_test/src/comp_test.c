 ...
 */
 ...
 comp_configuration COMP_Config;
 
 void SystemInit(void) 
 {
     NVIC_EnableIRQ(14);
     NVIC_EnableIRQ(15);
     COMP_Config.enable = 1;
     COMP_Config.received_irq_en = 1;
     COMP_Config.full_irq_en = 1;
     comp_config(COMP0, &COMP_Config);
 ...
 {
    comp_clear_irq(COMP0, 1, 0);
 }
 
 void __attribute__((used)) GPIO_INT_15_Handler(void)
 {
    comp_clear_irq(COMP0, 0, 1);
 }
