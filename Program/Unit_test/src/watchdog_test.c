...
watchdog_configuration watchdog_conf;

void SystemInit(void) {
      watchdog_conf.irq_en=1;
      watchdog_conf.resen=1;

      watchdog_config(CMSDK_WATCHDOG,&watchdog_conf);

      
}

int main(void) {
      watchdog_set_load(CMSDK_WATCHDOG,9898);
}
